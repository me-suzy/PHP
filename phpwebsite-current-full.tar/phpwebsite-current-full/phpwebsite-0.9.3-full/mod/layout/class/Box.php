...
class PHPWS_Layout_Box {
  ...
  function loadBoxInfo(){
	...
	extract ($result);
	if (!isset($this->ordered_list[$theme_var]["content"][$box_order]))
	  $this->ordered_list[$theme_var]["content"][$box_order] = $content_var;
	else {
	  $this->rebuildThemeBoxes($this->current_theme, $theme_var);
	  $this->loadBoxInfo();
	  return;
	}
  ...
  function buildThemeVarList(){
    if ($file = $GLOBALS["core"]->readFile($this->theme_dir. "transfers.tpl")){
      $vars = explode("\n", $file);
      $GLOBALS["core"]->dropNulls($vars);
      foreach ($vars as $row){
	$sub_row = explode(":", $row);
  ...
  function create_temp($mod_title, $content_var, $theme_var){
    $themeList = PHPWS_Layout::get_themes();
    foreach ($themeList as $theme)
      $this->_addBox($theme, $mod_title, $content_var, $theme_var);
  ...
  function _addBox($theme, $mod_title, $content_var, $theme_var=NULL, $home_only=NULL, $default_box=NULL, $default_pop=NULL){
    if ($GLOBALS["core"]->getOne("select * from mod_layout_box where theme='$theme' and content_var='$content_var' and theme_var='$theme_var'", TRUE))
      return;

    $defaultDir = $GLOBALS["core"]->home_dir . "themes/". $theme . "/defaults/";
  ...
  function moveTop($content_var){
    ...
    $left = $GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/layout/img/left.gif");
    $up = $GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/layout/img/up.gif");
    $dup = $GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/layout/img/dup.gif");

    $link_top = $GLOBALS["core"]->moduleLink($left, "layout", $left_info)
      . $GLOBALS["core"]->moduleLink($up, "layout", $up_info)
      . $GLOBALS["core"]->moduleLink($dup, "layout", $topInfo)
  ...
  function moveBottom($content_var){
    ...
    $right = $GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/layout/img/right.gif");
    $down = $GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/layout/img/down.gif");
    $ddown = $GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/layout/img/ddown.gif");

    $theme_var = $this->content_array[$content_var]["theme_var"];
    //    $box_order = $GLOBALS["core"]->moduleLink($this->content_array[$content_var]["box_order"], "layout", $bottomInfo);
    $box_order = $this->content_array[$content_var]["box_order"];
    $section = $this->ordered_list[$theme_var]["section"];
    $row = $this->ordered_list[$theme_var]["row"];

    $link_bottom = "<span class=\"smalltext\" style=\"background-color : white; color : black;\">$section : $row : $box_order</span> "
      . $GLOBALS["core"]->moduleLink($ddown, "layout", $bottomInfo)
      . $GLOBALS["core"]->moduleLink($down, "layout", $down_info)
      . $GLOBALS["core"]->moduleLink($right, "layout", $right_info);
  ...
  function shift_horz($content_var, $direction){
    ...
    if ($this->row_col[$shift_section][$current_row])
      ...
      $shift_row = $GLOBALS["core"]->maxKey($this->row_col[$shift_section]);
    ...
    if($horz_rows = $this->ordered_list[$horz_theme]["content"])
  ...
  function jump_theme($content_var, $theme_var, $direction){
    ...
    $max_section = $GLOBALS["core"]->maxKey($this->row_col);                  // Last section number

    $max_row = $GLOBALS["core"]->maxKey($this->row_col[$trans_section]);      // Last row number of current section
	...
	if ($jump_rows = $this->ordered_list[$jump_tvar]["content"]){
  ...
  function installModule($mod_title){
    $home_only = NULL;
    $themeList = PHPWS_Layout::get_themes();
    $modDir = $GLOBALS["core"]->source_dir . "mod/" . $GLOBALS["core"]->getModuleDir($mod_title) . "/conf/layout.php";
