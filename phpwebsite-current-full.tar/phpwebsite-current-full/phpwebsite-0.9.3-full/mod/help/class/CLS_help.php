...
class CLS_help
{
  ...
  function main_menu() {
    ...
    $myelements[0] = $GLOBALS["core"]->formHidden("module", "help");
    $myelements[0] .= $user_text . "<table border=\"0\"><tr><td>" . $GLOBALS["core"]->formSelect("reg_id", $opt_array) . "</td><td>";
    $myelements[0] .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Load Module"), "help_op");
    $myelements[0] .= $this->show_link("help", "main_menu") . "<br />";
    $myelements[0] .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Reload"), "help_op").$this->show_link("help", "reload")."</td></tr></table><br />";
    $content = $GLOBALS["core"]->makeForm("HELP_main_menu", "index.php", $myelements, "post", 0, 1);

    $myelements[0] = $GLOBALS["core"]->formHidden("module", "help");
    $myelements[0] .= $GLOBALS["core"]->formHidden("help_op", "act_deact_module");
    $myelements[0] .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Activate/Deactivate"), "bla");
    $myelements[0] .= $this->show_link("help", "act_deact") . "<br /><br />";
    $content .= $GLOBALS["core"]->makeForm("HELP2_main_menu", "index.php", $myelements, "post", 0, 1);

    $myelements[0] = $GLOBALS["core"]->formHidden("module", "help");
    $myelements[0] .= $GLOBALS["core"]->formHidden("help_op", "load_module");
    $myelements[0] .= $GLOBALS["core"]->formTextField("load_module", $_SESSION["translate"]->it("Module Name"),15)."&nbsp;";
    $myelements[0] .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Load Module"), "bla");
    $myelements[0] .= $this->show_link("help", "load_module");
    $content .= $GLOBALS["core"]->makeForm("HELP3_main_menu", "index.php", $myelements, "post", 0, 1);
  ...
  function edit_help_content() {
    ...
    $myelements[0] = $GLOBALS["core"]->formHidden("module", "help") . $GLOBALS["core"]->formHidden("reg_id", $this->reg_id) . $user_text;
    $myelements[0] .= $this->show_link("help", "edit_module") ."<br /><table border=\"0\">";

    foreach($this->help_content as $key=>$value) {
      $myelements[0] .= "<tr><td><b>$value[0]:</b>&nbsp;&nbsp;".$GLOBALS["core"]->formCheckBox("active[$key]", '1', "$value[3]", '1', "Active")."<br />";
      $myelements[0] .= $GLOBALS["core"]->formTextArea("$key","$value[2]",6,40)."</tr>";
    }
    ...
    $myelements[0] .= "</table>";
    $myelements[0] .= "<br />" . $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Save Data"), "help_op");
    $content .= $GLOBALS["core"]->makeForm("HELP_edit_menu", "index.php", $myelements, "post", 0, 1);
    $_SESSION["OBJ_layout"]->popbox($title, $content, NULL, "CNT_help") . "<br />";
  }
  ...
  function act_deact_module() {
    ...
    $myelements[0] .= $GLOBALS["core"]->formHidden("module", "help") . $user_text;
    $myelements[0] .= $GLOBALS["core"]->formHidden("help_op", "save_act_deact");
    $highlight = NULL;
    $result = $GLOBALS["core"]->sqlSelect("mod_help_reg");
    foreach($result as $value) {
      if($highlight)
	...
	$myelements[0] .= "<tr class=\"bg_light\">";
      ...
      $myelements[0] .= "<td>" . $value["mod_name"] . "</td><td>" . $GLOBALS["core"]->formRadio($value["reg_id"], '1', "1", $value["active"], "On" ) . $GLOBALS["core"]->formRadio($value["reg_id"], '0', "0", $value["active"], "Off" ) . "</td>";
      $GLOBALS["core"]->toggle($highlight);
    }
    $myelements[0] .= "</tr></table>";
    $myelements[0] .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Set Active"), "bla");
    $content .= $GLOBALS["core"]->makeForm("HELP_act_deact", "index.php", $myelements, "post", 0, 1);
    $_SESSION["OBJ_layout"]->popbox($title, $content, NULL, "CNT_help") . "<br />";
  }
  ...
  {
    $sql = "select reg_id from ".$GLOBALS["core"]->tbl_prefix."mod_help_reg where mod_name = '$module_name' and active = '1';";    
    $row_reg = $GLOBALS["core"]->quickFetch($sql);
    if($row_reg) {
      $sql = "select * from ".$GLOBALS["core"]->tbl_prefix."mod_help where reg_id = '".$row_reg["reg_id"]."' and label_name_id = '$label_name_id';";
      $row_h = $GLOBALS["core"]->quickFetch($sql);
	...
	return NULL;
      ...
      if($row_h["active"] == 1) {
	$source_http = NULL;
	$source_http = "http://" . $GLOBALS['core']->home_http;
	include($GLOBALS["core"]->source_dir."mod/help/conf/help_config.php");
	if($_SESSION["OBJ_user"]->js_on) {
	  $window_array = array(
				"type"=>"link",
				"url"=>"$source_http" . "index.php?module=help&amp;help_op=show_help&amp;module_name=$module_name&amp;label_name_id=$label_name_id&amp;hreg_id=".$row_reg["reg_id"],
				"label"=>"$help_graphic",
				"window_title"=>"help",
				"scrollbars"=>"yes",
				"width"=>"400",
	  ...
	  return "<a href=\"" . $source_http . "index.php?module=help&amp;help_op=show_help&amp;module_name=$module_name&amp;label_name_id=$label_name_id&amp;hreg_id=".$row_reg["reg_id"]."&amp;lay_quiet=1\" target=\"_blank\">$help_graphic</a>";
	}
	$help_link = $GLOBALS["core"]->js_insert("window", NULL, NULL, NULL, $window_array);
	return $help_link;
      }
    }
  }
  ...
  function show_popup($label_name_id) {
	...
	if ($result = $_SESSION["translate"]->dyn("mod_help", $key)){
	  ...
	  $tag_array["CONTENT"]=$GLOBALS["core"]->breaker($result["content"]);
	  $tag_array["STYLESHEET"]="http://".$_SESSION["OBJ_layout"]->theme_address."style.css";
	} else {
	  $tag_array["LABEL"]=$value[0];
	  $tag_array["CONTENT"]=$GLOBALS["core"]->breaker($value[2]);
	  $tag_array["STYLESHEET"]="http://".$_SESSION["OBJ_layout"]->theme_address."style.css";
	}
	if($_SESSION["OBJ_user"]->js_on)
	  $tag_array["CONTENT"] .= "<br /><br /><center><input type=\"button\" name=\"close\" value=\"".$_SESSION["translate"]->it("Close this Window")."\" onClick=\"parent.close()\"></center>";
      }
    
    echo $GLOBALS["core"]->processTemplate($tag_array, "help", "default.tpl");
  }
  ...
  function setup_help($mod_name, $helpFile = NULL) {
    if($helpFile === NULL) {
      if(file_exists($GLOBALS["core"]->source_dir . "mod/" . $mod_name . "/conf/help.php"))
        $helpFile = $GLOBALS["core"]->source_dir . "mod/" . $mod_name . "/conf/help.php";
      else
        return FALSE;
    ...
    $reg_id = $GLOBALS["core"]->sqlInsert($reg_data, "mod_help_reg", NULL, 1);
    $string = $GLOBALS["core"]->readFile($helpFile);
    ...
    foreach($array as $var_name) {
      ...
      $id = $GLOBALS["core"]->sqlInsert($help_data, "mod_help", "help_id");
