...
class PHPWS_Fatcat extends PHPWS_Fatcat_Forms{
  ...
  function setImageInfo($imageName, $type){
    $imageDir = $GLOBALS["core"]->home_dir . "images/fatcat/$type/$imageName";
  ...
  function linkToAdmin(){
    return $GLOBALS["core"]->moduleLink($_SESSION["translate"]->it("Admin Menu"), "fatcat", array("fatcat[admin]"=>"menu")); 
  }
  ...
  function savePic($postVar, $image_directory, $widthLimit=NULL, $heightLimit=NULL){
    ...
    if (!preg_match("/image\/(jpeg|gif|png|jpg|pjpeg)/i", $_FILES[$postVar]["type"])){
  ...
  function getModuleElementList($cat_id, $module_title){
    ...
    $pageIds = $GLOBALS["core"]->paginateDataArray($idList, "index.php?".$_SERVER["QUERY_STRING"], 10, TRUE, NULL, NULL, 20, TRUE);
	...
	$fatList["TOGGLE"] = " ";
      $fatcount++; 
      $template["ELEMENTS"] .= $GLOBALS["core"]->processTemplate($fatList, "fatcat", "genericElement.tpl");   
    }

    $template["TITLE"] = $modInfo["mod_pname"];
    $template["DIRECTORY"] = $pageIds[1];
    $template["NUMBER"] = $pageIds[2];
    $data = $GLOBALS["core"]->processTemplate($template, "fatcat", "genericList.tpl");
    return $data;
  }
