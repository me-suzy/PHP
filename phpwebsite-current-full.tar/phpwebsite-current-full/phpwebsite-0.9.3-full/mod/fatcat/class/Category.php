...
class PHPWS_Fatcat_Category {
  ...
  function createCategoryLinks($cat_id, $module_title=NULL, $noTemplate=TRUE, $noKids=TRUE, $noParent=TRUE){
    ...
    if ($noTemplate)
      ...
      return $GLOBALS["core"]->processTemplate($links, "fatcat", "links.tpl");
  }
  ...
  function createCategory($title, $description=NULL, $template=NULL, $image=NULL, $icon=NULL, $parent="0", $children=NULL){
    ...
    $title = $GLOBALS["core"]->parseInput($title, "none");
    
    $description = $GLOBALS["core"]->parseInput($description);
  ...
  function displayCategoryLink($cat_title, $cat_id, $module_title=NULL){
    ...
    $fatLink = $GLOBALS["core"]->moduleLink(strip_tags($cat_title), "fatcat", $link);
    return $fatLink;
  }
  ...
  function viewCategory($cat_id, $module_title=NULL){
    ...
    $image_address = $GLOBALS["core"]->home_http . "images/fatcat/images/";

    $cat = new PHPWS_Fatcat_Category($cat_id);

    $viewCategory["TITLE"] = $cat->title;
    if ($cat->description)
      $viewCategory["DESCRIPTION"] = $GLOBALS["core"]->parseOutput($cat->description);
    else
	...
	$viewCategory["IMAGE"] = $GLOBALS["core"]->imageTag($image_address.$cat->image["name"], $cat->title, $cat->image["width"], $cat->image["height"]);
    }

    $viewCategory["ICON"] = $cat->getIcon();

    //    $viewCategory["LINKBACK"] = $GLOBALS["core"]->link($_SERVER["HTTP_REFERER"], $_SESSION["translate"]->it("Go Back"));
    ...
    return $GLOBALS["core"]->processTemplate($viewCategory, "fatcat", "display/".$cat->template);
  }
  ...
  function getIcon($module_id=NULL, $allIcons=FALSE, $onlyIcons = FALSE, $module_title = NULL){
    $icon_address = $GLOBALS["core"]->home_http . "images/fatcat/icons/";
	...
	if ($cat->icon["name"]){
	  if (!file_exists($icon_directory . $cat->icon["name"])){
	    ...
	    $icon = $GLOBALS["core"]->imageTag($icon_address . $cat->icon["name"], $cat->title, $cat->icon["width"], $cat->icon["height"]);
	}
	else
	  return NULL;

	$link = $GLOBALS["core"]->moduleLink($icon, "fatcat", array("fatcat[user]"=>"viewCategory", "fatcat_id"=>$cat->cat_id));
	return $link;
    } else
  ...
  function getImage($module_id=NULL, $allImages=FALSE, $onlyImages = FALSE, $module_title = NULL){
	...
	if ($cat->image["name"]){
	  if (!file_exists($image_directory . $cat->image["name"])){
	    ...
	    $image = $GLOBALS["core"]->imageTag($image_address . $cat->image["name"], $cat->title, $cat->image["width"], $cat->image["height"]);
	}
	else
	  return NULL;

	$link = $GLOBALS["core"]->moduleLink($image, "fatcat", array("fatcat[user]"=>"viewCategory", "fatcat_id"=>$cat->cat_id));
	return $link;
    } else
