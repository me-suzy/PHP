...
class PHPWS_Approval{
  ...
  Function view(){
    $approvalFile = $GLOBALS["core"]->source_dir . "mod/" . $GLOBALS["core"]->getModuleDir($this->mod_title) . "/conf/approval.php";
  ...
  function admin(){
	...
	if($_SESSION["OBJ_user"]->js_on)
	  ...
	  $elementtpl["VIEW"] = $GLOBALS["core"]->moduleLink($_SESSION["translate"]->it("View"), "approval", array("approval_op"=>"view", "id"=>$data["id"], "lay_quiet"=>1), "index");
	...
	$sectiontpl["JOBS"]   .= $GLOBALS["core"]->processTemplate($elementtpl, "approval", "jobRows.tpl");
      }
      $approvetpl["MODULES"] .= $GLOBALS["core"]->processTemplate($sectiontpl, "approval", "section.tpl");
    }
    $content .= $GLOBALS["core"]->processTemplate($approvetpl, "approval", "approval.tpl");
    return $content;
  ...
  function viewLink($id, $mod_id){
    $window_array = array("type"=>"link",
			  "url"=>"http://" . PHPWS_HOME_HTTP . "index.php?module=approval&approval_op=view&id=". $id,
			  "label"=>$mod_id,
  ...
  function get($id){
    if ($row = $GLOBALS["core"]->sqlSelect("mod_approval_jobs", "id", $id))
      $GLOBALS["core"]->arrayToObject($row[0], $this);
    else
