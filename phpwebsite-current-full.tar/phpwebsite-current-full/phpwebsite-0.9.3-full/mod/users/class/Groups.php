...
class PHPWS_User_Groups extends PHPWS_User_Forms {
  ...
  function loadGroup($group_id){
    ...
    if (!($rows = $GLOBALS["core"]->sqlSelect("mod_user_groups", "group_id", $group_id)))
      exit("Error in loadGroup: Could not locate group in database.");
  ...
  function getMemberRights($user_id=NULL){
	...
	foreach ($group->permissions as $rightName=>$subRight){
	  ...
	  elseif (isset($permissions[$rightName])){
		...
		$permissions[$rightName] = $permissions[$rightName] + $subRight;
	    }
	  }
	}
      }
  ...
  function groupAction($mode){
    ...
    $this->description = $GLOBALS["core"]->parseInput($groupDesc);
  ...
  function deleteGroup($group_id, $confirm=NULL){
    if (!$confirm){
      ...
      $content .= $GLOBALS["core"]->moduleLink($_SESSION["translate"]->it("Yes"), "users", array("user_op"=>"deleteGroup", "confirm"=>"yes", "group_id"=>$group_id))."&nbsp;&nbsp;";
      $content .= $GLOBALS["core"]->moduleLink($_SESSION["translate"]->it("No"), "users", array("user_op"=>"manageGroups"));
  ...
  function removeGroupFromUser($user_id, $group_id){
    ...
    if ($user->groups)
      $update["groups"] = implode(":", $user->groups);
    else
      $update["groups"] = NULL;
