...
if (isset($lng_adm_op) && $_SESSION['OBJ_user']->allow_access("language")){
  ...
  if ($regLanguage){
    if (!$_SESSION["OBJ_lang"]->createLanguage($reg_opt_language))
      ...
      $CNT_lang["content"] .= $_SESSION["translate"]->it("The [var1] regular language table was created", $_SESSION["OBJ_lang"]->fullname($language))."!<br />";
  }
  if ($dynLanguage){
    if(!$_SESSION["OBJ_lang"]->createDynamic($dyn_opt_language))
      ...
      $CNT_lang["content"] .= $_SESSION["translate"]->it("The [var1] dynamic language table was created", $_SESSION["OBJ_lang"]->fullname($language))."!<br />";
  ...
  case "removeLanguageTableAction":
    if ($_POST["confirm_language_deletion"] && !$_POST["deny_language_deletion"])
