<?php
include ($GLOBALS["core"]->source_dir."mod/language/class/Dynamic.php");
...
class PHPWS_Language extends PHPWS_Dynamic{
  ...
  function getLanguageFile(){
    return file($GLOBALS["core"]->source_dir."mod/language/languages.txt");
  }
  ...
  function getRegularLanguages(){
    ...
    foreach ($sql as $table){
      if (preg_match("/mod_lang_\w\w$/", $table)){
  ...
  function export($language, $destination, $module="all"){
	...
	$finalFile .= $phrases;
      ...
      if ($module != "all"){
	if ($GLOBALS["core"]->writeFile($destination, $finalFile, 1, 1)){
	  return TRUE;
	}
	else
	  return FALSE;
  ...
  function import($filename, $dumpDups=TRUE){
    ...
    if ($language_array = file($filename)){
      $GLOBALS["core"]->dropNulls($language_array);
  ...
  function convert_language_file($filename){
    if (file_exists($filename)){
      $file = $GLOBALS["core"]->readFile($filename, 1);
      $lang_array = explode("\n", $file);
  ...
  function export_lang($language, $module){
    ...
    $destination = $GLOBALS["core"]->source_dir."mod/$mod_dir/lang/$module.$language.lng";
  ...
  function import_lang($language, $module){
    $mod_dir = $GLOBALS["core"]->getModuleDir($module);
    $GLOBALS["CNT_lang"]["content"] .= $this->link_to_admin()."<br /><br />";
    $file = $GLOBALS["core"]->source_dir."mod/$mod_dir/lang/$module.$language.lng";
    if (file_exists($file)){
	...
	$GLOBALS["CNT_lang"]["content"] .= $GLOBALS["core"]->ezTable($table, 5);
      } else
  ...
  function link_to_admin(){
    ...
    return $GLOBALS["core"]->link("index.php", $_SESSION["translate"]->it("Back to Admin"), "index", $vars);
  }
  ...
  function setUserLanguage($language){
    ...
    $GLOBALS["CNT_lang"]["content"] = $GLOBALS["core"]->link("index.php", $_SESSION["translate"]->it("Return to the user menu"), "index", array("module"=>"language","lng_usr_op"=>"user_admin"));
  }
  ...
  function user_admin(){
    ...
    if (!$this->langActive)
      ...
      $content .= $GLOBALS["core"]->formHidden(array("module"=>"language", "lng_usr_op"=>"setUserLanguage"))."\n";
      $content .= $GLOBALS["core"]->formSelect("user_language", $this->getRegularLanguages(), $_SESSION["translate"]->current_language, null, 1)." \n";
      $content .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Set My Default Language"));
      $content .= "</form>";
    }
  ...
  function admin(){
    ...
    $template["SET_DEFAULT"]      = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Set Default Language"), "set_default_lang");
    $template["EDIT"]             = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Edit Language"), "editLanguage");
    $template["CREATE"]           = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Create Language Table"), "createLanguageTable");
    $template["REMOVE"]           = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Remove Language Table"), "removeLanguageTableForm");
    $template["IMPORT_BUTTON"]    = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Import Language File"), "import_lang_file");
    $template["EXPORT_BUTTON"]    = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Export Language File"), "export_lang_file");
    $template["UPDATE_BUTTON"]    = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Update Language Settings"), "updateLangSettings");
    $template["MISSING"]          = $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Search for Missing Translations"), "langMissing");

    $template["LANG_SELECT"]      = $GLOBALS["core"]->formSelect("language", $this->available_languages, $this->fullname($this->default_language));
    $template["MODULE"]           = $GLOBALS["core"]->formSelect("lng_module", $mods, NULL, 1);

    $template["LANG_ACTIVE_OFF"]  = $GLOBALS["core"]->formRadio("langActive_switch", 0, $this->langActive)." ".$_SESSION["translate"]->it("Off");
    $template["LANG_ACTIVE_ON"]   = $GLOBALS["core"]->formRadio("langActive_switch", 1, $this->langActive)." ".$_SESSION["translate"]->it("On");
    $template["DYN_ACTIVE_OFF"]   = $GLOBALS["core"]->formRadio("dynActive_switch", 0, $this->dynActive)." ". $_SESSION["translate"]->it("Off");
    $template["DYN_ACTIVE_ON"]    = $GLOBALS["core"]->formRadio("dynActive_switch", 1, $this->dynActive). " ". $_SESSION["translate"]->it("On");
    $template["MARK_OFF"]         = $GLOBALS["core"]->formRadio("mark_switch", 0, $this->mark). " " . $_SESSION["translate"]->it("Off");
    $template["MARK_ON"]          = $GLOBALS["core"]->formRadio("mark_switch", 1, $this->mark). " ".$_SESSION["translate"]->it("On");
    $template["AUTO_OFF"]         = $GLOBALS["core"]->formRadio("auto_up_switch", 0, $this->auto_up) . " ". $_SESSION["translate"]->it("Off");
    $template["AUTO_ON"]          = $GLOBALS["core"]->formRadio("auto_up_switch", 1, $this->auto_up) . " ". $_SESSION["translate"]->it("On");
    $template["IGNORE_OFF"]       = $GLOBALS["core"]->formRadio("ignoreDefault_switch", 0, $this->ignoreDefault) . " " . $_SESSION["translate"]->it("Off");
    $template["IGNORE_ON"]        = $GLOBALS["core"]->formRadio("ignoreDefault_switch", 1, $this->ignoreDefault) . " " . $_SESSION["translate"]->it("On");
    ...
    $template["LANG_ACTIVE_HELP"] = $_SESSION["OBJ_help"]->show_link("language", "langActive");
    $template["DYN_ACTIVE_HELP"]  = $_SESSION["OBJ_help"]->show_link("language", "dynActive");
    $template["MARK_HELP"]        = $_SESSION["OBJ_help"]->show_link("language", "mark");
    $template["AUTO_HELP"]        = $_SESSION["OBJ_help"]->show_link("language", "auto");
    $template["IGNORE_HELP"]      = $_SESSION["OBJ_help"]->show_link("language", "ignore");
    $template["LANG_HELP"]        = $_SESSION["OBJ_help"]->show_link("language", "languageOptions");
    $template["IMP_EXP_HELP"]     = $_SESSION["OBJ_help"]->show_link("language", "importExport");

    if ($dyn_list){
      $template["DYN_HELP"]         = $_SESSION["OBJ_help"]->show_link("language", "dynAdmin");
	...
	foreach ($dynModules as $modInfo){
	  ...
	  if ($count = $this->countUntranslated($abbr, $moduleTable))
	    $untranslated_info = $GLOBALS["core"]->moduleLink($count, "language", $untrans_link);
	  else
	    $untranslated_info = "0";
	  
	  if ($count = $this->countUpdated($abbr, $moduleTable))
	    $updated_info = $GLOBALS["core"]->moduleLink($count, "language", $updated_link);
	  else
	    $updated_info = "0";
	  
	  $refresh = $GLOBALS["core"]->moduleLink($GLOBALS["core"]->imageTag($GLOBALS["core"]->source_http."mod/language/check.gif"), "language", array("module"=>"language", "lng_adm_op"=>"refreshDynamic", "language"=>$abbr, "table_name"=>$moduleTable));
      ...
      $template["DYN_LIST"] = $GLOBALS["core"]->ezTable($table2, 6, 0, 0, "100%", NULL, 1, "top");
    }

    $GLOBALS["CNT_lang"]["title"] = $_SESSION["translate"]->it("Language Administration") . $_SESSION["OBJ_help"]->show_link("language", "langAdmin");
    $content = 
       "\n<form action=\"index.php\" method=\"post\">"
       . $GLOBALS["core"]->formHidden(array("module"=>"language", "lng_adm_op"=>"adminMenuSelect"))
       . $GLOBALS["core"]->processTemplate($template, "language", "adminMenu.tpl")
       . "</form>\n";
    return $content;
  }
  ...
  function removeLanguageTableForm($language){
    ...
    $content .= "<form action=\"index.php\" method=\"post\">\n";
    $content .= $GLOBALS["core"]->formHidden(array("module"=>"language", "lng_adm_op"=>"removeLanguageTableAction", "language"=>$language));
    $content .= $GLOBALS["core"]->formCheckBox("confirm_language_deletion") ." " . $_SESSION["translate"]->it("Check this box to confirm the deletion").".<br />\n";
    $content .= $GLOBALS["core"]->formCheckBox("deny_language_deletion", 1, 1) ." " . $_SESSION["translate"]->it("Uncheck this box to confirm the deletion").".<br /><br />\n";
    $content .= $GLOBALS["core"]->formSubmit("Yes, remove ".$this->fullname($language)." language table")."\n";
    $content .= "</form>";
  ...
  function createLanguageTable(){
    $hidden = $GLOBALS["core"]->formHidden(array("module"=>"language", "lng_adm_op"=>"createLanguage")); 
    ...
    $title = $_SESSION["translate"]->it("Create Language Table") . $_SESSION["OBJ_help"]->show_link("language", "createTable");

    $content = "
<form action=\"index.php\" method=\"post\">
$hidden";
    $content .= $GLOBALS["core"]->formSelect("reg_opt_language", $languages)." ";
    $content .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Create Language Table"), "regLanguage")."<br /><br />";

    if (count($current_languages)){    
      $content .= $GLOBALS["core"]->formSelect("dyn_opt_language", $current_languages)." ";
      $content .= $GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Create Dynamic Table"), "dynLanguage")."<br /><br />";
    }
$content .= "</form>";
  ...
  function editLanguage($language){
    ...
    $GLOBALS["CNT_lang"]["title"] = $_SESSION["translate"]->it("Update [var1]", $this->fullname($language)) . $_SESSION["OBJ_help"]->show_link("language", "editLanguage");

    $content = $this->link_to_admin()."<br />
<form action=\"index.php\" method=\"post\">
".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Search for Missing Translations"), "lang_edit_com[search_for_missing]")."<br /><br />
".$GLOBALS["core"]->formTextField("lng_search_phrase", NULL, 30)."
".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Search for Phrase"), "lang_edit_com[search_phrase]")."<br /><br />
".$GLOBALS["core"]->formTextField("lng_search_translation", NULL, 30)."
".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Search for Translation"), "lang_edit_com[search_translation]")."<br /><br />";

$content .= $GLOBALS["core"]->formHidden(array("lng_adm_op"=>"edit_lang_action", "module"=>"language", "language"=>$language))."<hr />
...
<table cellpadding=\"4\">
  ...
  <tr>
   <td>".$GLOBALS["core"]->formTextField("lang_new_phrase")."</td><td>".$GLOBALS["core"]->formTextField("lang_new_trans")."</td>
   <td>".$GLOBALS["core"]->formSelect("lang_mod_title", $all_mods, "core", 1)."</td>
   <td>".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Insert Phrase"), "lang_edit_com[insert_phrase]")."</td>
  </tr>
</table>
";
...
";
  ...
  function edit_phrases($mode=NULL){
	...
	$i++;
	extract($value);
	$GLOBALS["core"]->toggle($color, " class=\"bg_light\"");
	$this->phrases[$i] = "
  <tr".$color.">
    <td align=\"center\">".$GLOBALS["core"]->formCheckBox("lng_edit_id[$phrase_id]")."</td>
    <td>".$GLOBALS["core"]->formSelect("lng_edit_module[$phrase_id]",$mod_array, $module, 1)."</td>
    <td>";
	if ($mode=="missing")
	  $this->phrases[$i].= $GLOBALS["core"]->formHidden("lng_edit_phrase[$phrase_id]", $phrase)."<b>$phrase</b>";
	else
	  if (strlen($phrase) > 40){
	    $rows = floor($phrase / 40) + 2;
	    $this->phrases[$i] .= $GLOBALS["core"]->formTextArea("lng_edit_phrase[$phrase_id]", $phrase, $rows, 40);
	  }
	  else
	    $this->phrases[$i] .= $GLOBALS["core"]->formTextField("lng_edit_phrase[$phrase_id]", $phrase, 40);
	$this->phrases[$i] .= "</td>
    <td>";

	if (strlen($translation) > 40){
	  $rows = floor($translation / 40) + 2;
	  $this->phrases[$i] .= $GLOBALS["core"]->formTextArea("lng_edit_translation[$phrase_id]", $translation, $rows, 40);
	}
	else
	$this->phrases[$i] .=  $GLOBALS["core"]->formTextField("lng_edit_translation[$phrase_id]", $translation, 40);

    $this->phrases[$i] .= "</td>
    <td>".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Delete"), "lng_drop_phrase[$phrase_id]")."</td>
  </tr>";
      }
  ...
  function list_phrases($mode=NULL){
    $data = $GLOBALS['core']->paginateDataArray($this->phrases, "./index.php?module=language&amp;lng_adm_op=list_phrases", 20, 1, array("<b>[ ", " ]</b>"), NULL, 20);

    $GLOBALS["CNT_lang"]["title"] = $_SESSION["translate"]->it("Search Result for [var1]", $this->fullName($this->current_language)) . $_SESSION["OBJ_help"]->show_link("language", "searchResult");
...
".$GLOBALS["core"]->formHidden(array("module"=>"language", "lng_adm_op"=>"edit_phrase_action", "language"=>$this->current_language, "mode"=>$mode))."
...
".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Update Checked"))."
".$GLOBALS["core"]->js_insert("check_all", "phrases", NULL, TRUE)."</div>
</form><br />";
  ...
  function insert_phrase($language, $phrase, $translation, $module){
    ...
    if (!isset($this->dictionary[$phrase]) && strlen($language) && strlen($phrase) && strlen($translation) && strlen($module)){
...
".$GLOBALS["core"]->formHidden("module", "language")."
".$GLOBALS["core"]->formHidden("lng_adm_op", "mass_insert")."
".$GLOBALS["core"]->formHidden("lng_upd_lang", $language)."
<table cellpadding=\"6\" width=\"100%\">
  <tr>
    ...
    foreach ($missing as $phrase_id=>$value){
      ...
      $content .= "
  <tr>
    <td align=\"center\">".$GLOBALS["core"]->formCheckBox("lng_add_comp_allow[]", $i)."</td>
    <td><b>$module</b>".$GLOBALS["core"]->formHidden("lng_add_modname[$i]", $module)."</td>
    <td>";

      if ($lock_phrase)
	$content .= $GLOBALS["core"]->formHidden("lng_add_phrase[$i]", $phrase)."<b>$phrase</b>";
      else
	$content .= $GLOBALS["core"]->formTextField("lng_add_phrase[$i]", $phrase, 30);

      $content .= "</td>
    <td>".$GLOBALS["core"]->formTextField("lng_add_trans[$i]", $translation, 30)."</td>
  </tr>";
    }

    $content .= "
</table>
".$GLOBALS["core"]->formSubmit($_SESSION["translate"]->it("Insert Checked"))." ".$GLOBALS["core"]->js_insert("check_all", "multi")."
</form>";
  ...
  function uninstallLanguages($moduleDir){
	...
	$GLOBALS["core"]->sqlDelete(PHPWS_Dynamic::dynamicTable($language), "module", $mod_title);
      }
    }
  ...
  function installLanguages($moduleDir){
    ...
    if (!($files = $GLOBALS["core"]->readDirectory($directory)))
