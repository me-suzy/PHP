...
class PHPWS_ControlPanel {
  ...
  function PHPWS_ControlPanel() {
    $sql = "SELECT id FROM " . $GLOBALS['core']->tbl_prefix . "mod_controlpanel_tab ORDER BY taborder";
  ...
  function display() {
    ...
    if(is_array($this->_tabs) && (sizeof($this->_tabs) > 0)) {
      $tabTags['PHPWS_SOURCE_HTTP'] = "http://" . PHPWS_SOURCE_HTTP;
      foreach($this->_tabs as $id => $title) {
	/* checking to see what was the current tab clicked */
	if($this->_currentTab == $id) {
	  $tabTags['TITLE'] = $title;

	  $panelTags['TABS'][] = $GLOBALS['core']->processTemplate($tabTags, "controlpanel", "tab/active.tpl");
	} else {
	  /* only create the link if it is not the current tab */
	  $tabTags['HREF'] = "./index.php?module=controlpanel&amp;CP_TAB=" . $id;
	  $tabTags['TITLE'] = $title;

	  $panelTags['TABS'][] = $GLOBALS['core']->processTemplate($tabTags, "controlpanel", "tab/inactive.tpl");
	}
      }
      ...
      $GLOBALS['CNT_controlpanel']['content'] = $GLOBALS['core']->processTemplate($panelTags, "controlpanel", "panel.tpl");
