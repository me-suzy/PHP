 ...
 * functions.
 * 
 * @version $Id: Array.php,v 1.29 2003/03/24 14:56:39 matt Exp $
 * @author  Matt McNaney <matt@NOSPAM.tux.appstate.edu>
 * @author  Adam Morton <adam@NOSPAM.tux.appstate.edu>
 * @package Core
 */
class PHPWS_Array extends PHPWS_Text
{
  /**
  ...
  function moveElement ($array, $index=NULL, $element=NULL, $moveup=true, $assoc_array=true) {
    ...
    if (!is_array($array))
      exit("moveElement received a/an ".gettype($array_var)." variable, not an array<br />");
    /* If only the value was given, search for the key & delete it */
    if ($index==NULL) {
      $index = array_search($element, $array);
      if (!$index)
	exit("moveElement received ".gettype($array_var) . ", a value that doesn't exist in the array.<br />");
    }
    $element = $array[$index];
  ...
  function paginateDataArray($content, $link_back, $default_limit=10, $make_sections=FALSE, $curr_sec_decor=NULL, $link_class=NULL, $break_point=20, $return_array=FALSE){
	...
	for($x = 1; $x <= $sections; $x++){
	  if($x == $current_section){
	    ...
	    $nav_links .= "&#160;<a href=\"" . $link_back . "&amp;PDA_limit=" . $limit . "&#38;PDA_start=" . ($limit * ($x - 1)) . "&#38;PDA_section=" . $x . "\" class=\"" . $link_class . "\" alt=\"" . $x . "\">" . $x . "</a>&#160;\n";
	  }
	}
	...
	$nav_links .= "<a href=\"" . $link_back . "&amp;PDA_limit=" . $limit . "&#38;PDA_start=" . ($start + $limit) . "&#38;PDA_section=" . ($current_section + 1) . "\" class=\"" . $link_class . "\" alt=\"&#62;&#62\">&#62;&#62;</a>\n";
