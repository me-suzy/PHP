 ...
 *
 * @version $Id: Database.php,v 1.80 2003/07/16 15:12:59 matt Exp $
 * @author  Matt McNaney <matt@NOSPAM.tux.appstate.edu>
 * @author  Adam Morton  <adam@NOSPAM.tux.appstate.edu>
 * @author  Steven Levin <steven@NOSPAM.tux.appstate.edu>
 * @package Core
 */
class PHPWS_Database extends PHPWS_Array {
  /**
   ...
   * @access   public
   */
  function loadDatabase($configFile=NULL, $suppress_error=FALSE) {
    if (isset($this->db))
      $this->db->disconnect();
  ...
  function query($sql, $addTablePrefix=FALSE, $error_pass=FALSE){
	...
	if ($error_pass) return FALSE;
	else exit($sql_result->getMessage() . "<br />" . $sql_result->userinfo);
  ...
  function dbReady($value=NULL) {
    if (is_array($value) || is_object($value))
      return PHPWS_Database::dbReady(serialize($value));
  ...
  function makeWhere($match, $compare, $and_or) {
	...
	if (is_array($and_or)) {
      ...
      if (is_array($compare)) {
	if ($compare[$column_name]) $sql .= $column_name." ".$compare[$column_name]." ".$column_value;
  ...
  function sqlCreateIndex($table_name, $columns){
    ...
    $sql = "ALTER TABLE $table_name ADD INDEX (" . implode(", ", $columns) . ")";
    if ($this->query($sql)) return TRUE;
    else return FALSE;
  ...
  function sqlImport($filename, $write = TRUE, $suppress_error=FALSE){
    ...
    if (file_exists($filename))	{
      ...
      $file = $this->readFile($filename, 1);
      $sql_array = $this->sentence($file);
