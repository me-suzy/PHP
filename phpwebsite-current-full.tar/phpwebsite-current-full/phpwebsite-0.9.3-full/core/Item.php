 ...
 *
 * @version $Id: Item.php,v 1.26 2003/07/03 17:00:14 adam Exp $
...
class PHPWS_Item {
  ...
  function init($row=NULL) {
	...
	$error = "No database result returned for ID : Table  " . $this->_id . " : " . $this->_table;
  ...
  function commit($extras=NULL, $supressSets=FALSE) {
	...
	$error = "The datebase update was unsuccessful.";
	return new PHPWS_Error("core", "PHPWS_Item::commit()", $error, "exit", 1);
      }
    } else {
	...
	$error = "The database intsert was unsuccessful.";
	return new PHPWS_Error("core", "PHPWS_Item::commit()", $error, "exit", 1);
      }      
    }
  } // END FUNC commit
  ...
  function setLabel($label = NULL) {
    if($label) {
      $this->_label = $GLOBALS['core']->parseInput($label);
    } else {
