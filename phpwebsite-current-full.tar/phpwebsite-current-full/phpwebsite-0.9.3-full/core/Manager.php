 ...
 *
 * @version $Id: Manager.php,v 1.54 2003/07/03 17:00:14 adam Exp $
...
class PHPWS_Manager {
  ...
  function init() {
    ...
    $config = $GLOBALS['core']->source_dir . "mod/" . $this->_module . "/conf/manager.php";
  ...
  function getList($listName, $title=NULL, $makeForm=TRUE, $overRideOp=NULL) {
    ...
    $themeModuleRowTpl = $GLOBALS['core']->home_dir . "theme/" . $this->_module . "/" . $this->_templates[$this->listName] . "/row.tpl";
    $moduleRowTpl = $GLOBALS['core']->source_dir . "mod/" . $this->_module . "/templates/" . $this->_templates[$this->listName] . "/row.tpl"; 
    $themeCoreRowTpl = $GLOBALS['core']->home_dir . "theme/core/defaultRow.tpl";
    $coreRowTpl = $GLOBALS['core']->source_dir . "templates/defaultRow.tpl";

    $themeModuleListTpl = $GLOBALS['core']->home_dir . "theme/" . $this->_module . "/" . $this->_templates[$this->listName] . "/list.tpl";
    $moduleListTpl = $GLOBALS['core']->source_dir . "mod/" . $this->_module . "/templates/" . $this->_templates[$this->listName] . "/list.tpl"; 
    $themeCoreListTpl = $GLOBALS['core']->home_dir . "theme/core/defaultList.tpl";
    $coreListTpl = $GLOBALS['core']->source_dir . "templates/defaultList.tpl";
    ...
    if(is_array($this->_listPaging[$this->listName])) {
	...
	switch($overRide) {
	case 0: 
	  $listTags[$key1] .= "<a href=\"./index.php?module=" . $this->_module . "&amp;" . $request . "&amp;PHPWS_MAN_LIST=" . $this->listName . "&amp;PHPWS_MAN_COLUMN=" . $listColumn . "&amp;PHPWS_MAN_ORDER=1" . $anchor . "\">";
	  $listTags[$key1] .= "<img src=\"http://" . PHPWS_SOURCE_HTTP . "core/img/manager/sort_none.png\" border=\"0\" /></a>";
	  break;
	  
	case 1:
	  $listTags[$key1] .= "<a href=\"./index.php?module=" . $this->_module . "&amp;" . $request . "&amp;PHPWS_MAN_LIST=" . $this->listName . "&amp;PHPWS_MAN_COLUMN=" . $listColumn . "&amp;PHPWS_MAN_ORDER=2" . $anchor . "\">";
	  $listTags[$key1] .= "<img src=\"http://" . PHPWS_SOURCE_HTTP . "core/img/manager/up_pointer.png\" border=\"0\" /></a>";
	  break;
	  
	case 2:
	  $listTags[$key1] .= "<a href=\"./index.php?module=" . $this->_module . "&amp;" . $request . "&amp;PHPWS_MAN_LIST=" . $this->listName . "&amp;PHPWS_MAN_COLUMN=" . $listColumn . "&amp;PHPWS_MAN_ORDER=0" . $anchor . "\">";
	  $listTags[$key1] .= "<img src=\"http://" . PHPWS_SOURCE_HTTP . "core/img/manager/down_pointer.png\" border=\"0\" /></a>";
	  break;
	...
	$listTags["LIST_ITEMS"] .= $GLOBALS["core"]->processTemplate($rowTags, "core", $rowTpl, FALSE);
      }
	...
	if($this->_listPaging[$this->listName]["section"]) {
	...
	$listTags["TOGGLE_ALL"] = $GLOBALS['core']->js_insert("check_all", "PHPWS_MAN_LIST_" . $this->listName);
	
	/* Add hidden variable to designate the current module */
	$elements[0] = PHPWS_Form::formHidden("module", $this->_module);
	$elements[0] .= $GLOBALS["core"]->processTemplate($listTags, "core", $listTpl, FALSE);
	...
	$content = $GLOBALS["core"]->processTemplate($listTags, "core", $listTpl, FALSE);
      }
  ...
  function getItems($ids=NULL, $filterGroups=FALSE, $everything=FALSE) {
	...
	$sql .= " FROM " . $GLOBALS["core"]->tbl_prefix . $table;
      } else {
  ...
  function _getIds() {
    ...
    $sql = "SELECT id FROM " . $GLOBALS['core']->tbl_prefix . $table;
  ...
  function _doMassUpdate($column, $value) {
    if(is_array($_REQUEST["PHPWS_MAN_ITEMS"]) && sizeof($_REQUEST["PHPWS_MAN_ITEMS"]) > 0) {
      ...
      $sql = "UPDATE " . $GLOBALS["core"]->tbl_prefix . $this->_table .
