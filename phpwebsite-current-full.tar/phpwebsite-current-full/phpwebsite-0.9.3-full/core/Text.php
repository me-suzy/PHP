 ...
 * 
 * @version $Id: Text.php,v 1.69 2003/07/03 17:39:14 matt Exp $
 * @author  Matthew McNaney <matt@NOSPAM.tux.appstate.edu>
 * @author  Adam Morton <adam@NOSPAM.tux.appstate.edu>
 * @author  Steven Levin <steven@NOSPAM.tux.appstate.edu>
 * @package Core
 */
class PHPWS_Text extends PHPWS_Form {
  ...
  function breaker($text){
    ...
    $text_array = PHPWS_Core::sentence($text);
  ...
  function ezTable($data_array, $cellpadding=0, $cellspacing=0, $border=0, $width=0, $th=FALSE, $toggle=FALSE, $valign=NULL, $class=NULL){
    ...
    if ($high_column){
      ...
      foreach ($data_array as $row_array){
        if ($toggle)
          PHPWS_Core::toggle($bg, " class=\"bg_light\"");
  ...
  function moduleLink($title, $module=NULL, $getVars=NULL, $target=NULL){
    $link = "<a href=\"http://" . PHPWS_HOME_HTTP;
  ...
  function link($address, $text, $type=NULL, $get_var=NULL, $target=NULL, $onclick=NULL){
    if ($type == "local") $address = "http://" . $this->source_http . $address;
