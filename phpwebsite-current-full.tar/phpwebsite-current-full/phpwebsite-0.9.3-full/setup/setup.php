...
CREATE TABLE mod_lang_".strtoupper($install_language)." (
  ...
  KEY module (module)
   ...
   if (isset($_POST["extraMods"]) && count($_POST["secModules"])){
   ...
   echo $core->moduleLink($_SESSION["translate"]->it("Go to my installation"));
 ...
 if ($db_connect == FALSE){
   ...
   $error = 1;
     ...
     if ($table_check && !$login && !$table_prefix){
...
<form action=\"".$our_dir."setup.php\" method=\"post\">"
     . $core->formHidden(array("install_step"=>1, "check_session"=>1, "hash"=>$install_hash))
     . $_SESSION["translate"]->it("Welcome to the phpWebSite installation utility") . "!<br />" . $_SESSION["translate"]->it("Please enter your installation password to continue.") . "<br /><br />" . $core->formPassword("install_password") . " " . $core->formSubmit($_SESSION["translate"]->it("Continue")) . "
</form>";
 }
...
function createUserForm(){
  ...
  $content .= $core->formHidden(array("hash"=>$GLOBALS["install_hash"], "install_step"=>1, "login"=>1));
  $content .= "
<table cellpadding=\"4\" cellspacing=\"1\" bgcolor=\"#000000\" border=\"0\">
  <tr><td bgcolor=\"#dddddd\"><b>" . $_SESSION["translate"]->it("Username") . ":</b></td><td bgcolor=\"#dddddd\">" . $core->formTextField("username", (isset($_POST["username"])) ? $_POST["username"] : NULL) ."</td></tr>
  <tr><td bgcolor=\"#ffffff\"><b>" . $_SESSION["translate"]->it("Password") . ":</b></td><td bgcolor=\"#ffffff\">". $core->formPassword("pass1") . "</td></tr>
  <tr><td bgcolor=\"#dddddd\"><b>" . $_SESSION["translate"]->it("Password Confirmation") . ":</b></td><td bgcolor=\"#dddddd\">" . $core->formPassword("pass2") . "</td></tr>
  <tr><td bgcolor=\"#ffffff\"><b>" . $_SESSION["translate"]->it("Email Address") . ":</b></td><td bgcolor=\"#ffffff\">" . $core->formTextField("email", (isset($_POST['email'])) ? $_POST['email'] : NULL) . "</td></tr>
</table><br />
" . $core->formSubmit($_SESSION["translate"]->it("Continue")) ."
</form>";
  return $content;
}
...
function submitUserInfo(){
  ...
  if (!$core->isValidInput($_POST["username"])){
...
function installSecModules($required){
  $core = $GLOBALS["core"];
  if (!($dir = $core->readDirectory($core->source_dir."mod/", 1)))
    return NULL;

  foreach ($dir as $mod_dir_name){
    $filename = $core->source_dir."mod/$mod_dir_name/conf/boost.php";
  ...
  if (count($mods)){
    ...
    $content .= "<form action=\"setup.php\" method=\"post\" name=\"modules\">"
       . $core->formHidden("install_step", "2") . "\n";
    
    foreach ($mods as $modDir=>$modName)
      $content .= $core->formCheckBox("secModules[]", $modDir) . " $modName<br />\n";

    $content .= $core->formButton($_SESSION["translate"]->it("Toggle All"), "toggle", "CheckAll();") .
       "<br /><br />";
    $content .= $core->formSubmit($_SESSION["translate"]->it("Install Selected Modules"), "extraMods") .
       "&#160;" . $core->formSubmit($_SESSION["translate"]->it("Install Core Only")) . "\n"
       . "</form>";
