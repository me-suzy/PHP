...
include("../mod/boost/class/Boost.php");
...
Make sure to check your config.php file for the correct password.";
  ...
  if (!$_SESSION["passwordAccepted"]){
    ...
    $GLOBALS["content"] .= "Install Password: " . $core->formPassword("password") . "<br />\n"
       . $core->formSubmit("Go");
...
function done(){
  ...
  echo PHPWS_Core::parseOutput($GLOBALS["content"]);
  exit();
}
...
function deactivateModule(){
  ...
  if (!$core->sqlTableExists("modules")){
...
<form action=\"repair.php\" method=\"post\">\n"
     . $core->formHidden("question", "deactivateModule") ."\n"
     . $core->formHidden("changeStatus", "1") ."\n
<table cellpadding=\"6\" border=\"1\">\n";
  
  foreach ($modules as $modInfo){
    extract($modInfo);
    $GLOBALS["content"] .= "<tr><td width=\"20%\"><b>$mod_pname</b></td>\n<td>";
    $GLOBALS["content"] .= $core->formRadio("$mod_title", "off", $active) . " Off ";
    $GLOBALS["content"] .= $core->formRadio("$mod_title", "on", $active) . " On</td>\n</tr>\n";
  }
  $GLOBALS["content"] .= "</table><br />" . $core->formSubmit("Go") . "\n</form>";
