...
if (!($disable_session)) {
    ...
    if (!empty($_SESSION[user_id])) {
        ...
        $query = "SELECT UNIX_TIMESTAMP(last_refresh_time) as last_refresh_time FROM sessions WHERE unique_session_num=$_SESSION[unique_session_num]";
...
function displayMenuQueue()
{
    ...
    else 
    {
        if($_SESSION[perms][p_unvote]) {
            ...
            $unvote_query = "SELECT DISTINCT song_id from queue where user_id=$_SESSION[user_id]";
        ...
        while ($row = mysql_fetch_object($result))
        {
            ...
            echo "<li>";
            echo "<a href=\"who.php?song_id=$row->song_id\" class=\"menu\">$queueText</a> (";
            echo display_time($row->seconds);
            echo ")\n";
            if(($_SESSION[perms][p_unvote]) AND (in_array($row->song_id, $allowed_unvotes))) {
                echo "<a href=\"vote.php?song_id=$row->song_id&action=unvote\"><img border=0 src=\"images/unvote.png\"></img></a><br>";
            }
            echo "</li>";
        }
...
function timeLeft()
{
    $kweerie        = "SELECT songs.song_id,artist_name,songtitle,started,TIME_TO_SEC(length) as length from np LEFT JOIN songs using(song_id)
        LEFT JOIN artists on songs.artist_id=artists.artist_id LEFT JOIN
    ...
    $length         = $row[length];
    if ($length == 0) {
        return -5000;  // return a really low number so it doesn't display anything
    }
    $end            = $started + $length;
...
{
    $kweerie        = "SELECT
        np.play_id,wasrandom,songs.song_id,artist_name,songtitle,started,TIME_TO_SEC(length)
        as length from np LEFT JOIN songs using(song_id) LEFT JOIN artists on
    ...
    if ($length == 0) {     // if the song has no viable length, forget about the clock
        $secondsLeft = NULL;
        $length_str = "No duration available";
    }
    else {
    ...
    if ($song_id > 0)
    {
        if (($row[wasrandom]==1) AND ($_SESSION[perms][p_random_skip])) {
            echo "<a href=\"skip_random.php\" title=\"This randomly chosen song really blows\">(<b><u>Skip Song</b></u>)</a><br>";
        }
        else if ($row[wasrandom]==1 AND empty($_SESSION[perms][p_random_skip])) {
            echo "(Random song)";
        }
        echo "<a href=\"who_play.php?play_id=$play_id\" title=\"Who Played That?\">$artist - $songtitle</a><br>Song Length: ($length_str)";
...
ENDSCRIPT;
        ...
        if ($_SESSION[perms][p_select_edit] OR $_SESSION[perms][p_select_vote]) { // for now but later we check all p_selects* TODO
            echo "<form name=\"songs\" action=\"admin_edit_record.php\" method='get'>";
        ...
        $hidden_fields = array("album_id", "status", "update_id3", "genre_id", "artist_id", "song_id", "play_id", "history_id", "mood_id", "unformatted_timestamp");
        ...
        while ($song = mysql_fetch_object($result))
        {
            ...
            if ($_SESSION[perms][p_select_edit] ||
                    ...
                    $_SESSION[perms][p_download]) {
                //       changed to save position in search list
                echo "<td><nobr><a title=\"$song->filename\" href=\"vote.php?action=vote&song_id=$song->song_id\"><img src=\"images/vote.png\" border=\"0\"></a>";
                echo " [";
                //                      echo "<a href=\"lyrics.php?song_id=$song_id\" title=\"Lyrics\">L </a>";
                if ($_SESSION[perms][p_select_edit]) {
                    echo "<a href=\"admin_edit_record.php?checkbox_action=edit&song_id=$song->song_id\" title=\"Edit\"><img src=\"images/edit.png\" border=\"0\"></a> ";
                }
                if ($_SESSION[perms][p_download]) {
                    echo "<a href=\"download.php?song_id=$song->song_id\" title=\"Download\"><img src=\"images/save.png\" border=\"0\"></a> ";
                }
                ...
                if ($_SESSION[perms][p_select_edit]) {
                    ...
                    if($song->status=='delete')
                        echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=normal\" title=\"Undelete\">UnDel </a>";
                    elseif($song->status=='normal')
                    {
                        echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=delete\" title=\"Delete\"><img src=\"images/delete.png\" border=\"0\"></a>";
                        echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=hide\" title=\"Hide\">H </a>";
                        echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=readtag\" title=\"Read from ID3 tags on file\">R </a>";
                        if($song->update_id3==1) {
                            // This songs ID3 tag needs
                            // updating!
                            echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=writetag\" title=\"Please forgive the blink tag, but this audio file's tag needs updating\"><blink>*</blink>W<blink>*</blink> </a>";
                        }
                        else {
                            echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=writetag\" title=\"Write ID3 tag back to file\">W </a>";
                        }
                    }
                    elseif($song->status=='offline')
                    {
                        echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=delete\"
                            title=\"Delete\">Del </a>";
                    }
                    elseif($song->status=='hide')
                        echo "<a href=\"admin_db_action.php?song_id=$song->song_id&action=normal\"
...
function nrOfSongs()
{
    return mysql_result( mysql_query("SELECT count(*) as aantal from songs"),0,aantal);
}
