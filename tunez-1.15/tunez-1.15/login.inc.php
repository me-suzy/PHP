...
function login() {
    $query = "SELECT user_id FROM users WHERE user=\"$_POST[user]\" AND pw=PASSWORD(\"$_POST[pw]\")";
...
function logout() {
    ...
    $query = "UPDATE sessions SET status='logged_out',logout_time=NOW(),saved=0 WHERE unique_session_num=\"$unique_session_num\"";
...
#setCookie("PHPSESSID", "");
    $query = "UPDATE sessions SET status='timed_out',logout_time=NOW(),saved=0 where session_id=\"$session_id\"";
    mysql_query($query) or die(mysql_error());
    session_unset();
    return $_SERVER[HTTP_REFERER];
}
