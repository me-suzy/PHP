...
function dovote($song_ids, $override=0) {
    ...
    if (($_SESSION[user_id] < 1) && (!($override)))
    {
    ...
    foreach ($song_ids as $song_id) {
        $kweerie = "SELECT * from queue WHERE song_id = '$song_id' and user_id = '$_SESSION[user_id]'";
...
function newvote($song_ids, $override=0) {
    ...
    $query = "SELECT vote_type,mode,period,votes,votes_banked FROM voting_rights where who='user' AND ug_id=$_SESSION[user_id]";
    ...
    foreach ($song_ids as $song_id) {
        $query = "SELECT * from queue WHERE song_id = '$song_id' and user_id = '$_SESSION[user_id]'";
        $result = tunez_query($query);
        if ((mysql_num_rows($result) > 0) && (!($override)))
        {
            $_SESSION[messageTitle] = "Voting not allowed!";
...
function retabulate_account($who, $ug_id) {
    ...
    if ($number_of_paydays) {
        ...
        $query = "UPDATE voting_rights set votes_banked = $new_votes_banked, time_added = NOW() WHERE vr_id = $row->vr_id";
        $result = tunez_query($query);
    }
}
