...
if(empty($_POST)) {
    ...
    else
    {
        ...
        if($authenticate)
        {
            ...
            $query = "INSERT INTO access (user_id, group_id) VALUES ($bestowed_user_id, -1)";
            ...
            $content .= "Please go to http://$_SERVER[SERVER_NAME]/confirm.php?confirmcode=$code&user_id=$bestowed_user_id to activate your account.";

            $messageTitle="Account Created";
            $messageBody="Your request is being processed.  A confirmation e-mail will be sent to $email shortly with directions on how to complete the registration process.";

            $query = "INSERT INTO pendingusers (pending_user_id, confirmation_code, timestamp) VALUES (\"$bestowed_user_id\", \"$code\", \"$time\")";
            $result = tunez_query($query);
            ...
            $text = "$user, you must now activate your account by clicking on the URL sent in the email";
        }
        else {
            // Create an entry for them in the users table
            $kweerie = "INSERT INTO users VALUES ('', '$user', PASSWORD('$pw'), '$email')";
            ...
            // Add them to the default group
            $bestowed_user_id = mysql_insert_id();
            $query = "INSERT INTO access (user_id, group_id) VALUES ($bestowed_user_id, $default_group_id)";
            $result = tunez_query($query);
            $_SESSION[unique_session_num] = insert_session($bestowed_user_id, session_id(), 0, NULL);
