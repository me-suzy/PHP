...
if (empty($_POST))
{
    ...
    $query = "SELECT * FROM groups LEFT JOIN permissions ON groups.group_id = permissions.ug_id WHERE permissions.who = 'group' AND groups.group_id=" . $_GET[edit_group_id];
        $result=tunez_query($query);

        $query = "SELECT * FROM users LEFT JOIN access ON users.user_id=access.user_id WHERE access.group_id=" . $_GET[edit_group_id];
...
<div class="formdiv">
<form action="admin_groups_edit.php" method="post">

<p>
<table border=0 cellspacing=1 cellpadding=1> 
<tr>
<td valign="top"><b>Group Permissions<br> <input type="checkbox" name="modpermis"> <i>Modify Permissions</i></b></td>
<td>
<table border=0>
<?php
...
foreach ($member as $currentmember) {
        if (!empty($currentmember[user])) {
            echo "<a href=\"admin_users_edit.php?edit_user_id=" . $currentmember[user_id] . " \" >" . $currentmember[user] . "</a>";
            echo "&nbsp;&nbsp;[<a href=\"admin_ug_rem.php?rm_group_id=" . $_GET[edit_group_id] . "&rm_user_id=" . $currentmember[user_id] . "\">del</a>]<br>";
        }
}
?>
</table>
