...
if (empty($_SESSION[user_id])) {
    ...
    echo "<p>Tunez doesn't know who you are. If you haven't used Tunez before, this might be a good time to <a href=signup.php class=dik>Sign up!</a> to get an account. You need it to vote.<BR><BR>If you already have an account, sign in using the form on the right</p>";
}

if (preg_match("/shout/i", $mode) || preg_match("/ices/i", $mode)) {
    echo "<h4>Icecast information</h4>";
    echo "<p>This installation is set up to use streaming with icecast.<BR>";
    echo "Click here to connect your audio player to the icecast server: <a href=\"icecast.php\">$icecast_URL</a></p>";
}
...
if (sizeof($randoms) > 0) {
    ...
    $kweerie	= "SELECT $typical_song_select FROM songs LEFT JOIN artists on
        ...
        songs.album_id=albums.album_id WHERE ( song_id=$choices ) AND $show_active ORDER BY RAND() LIMIT 10";
