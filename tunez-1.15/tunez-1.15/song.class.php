...
class song {
    ...
    // non-SQL variables:
    var $root_path; // this will be one of the entries in $mp3dir.
    var $path;
...
#print $query . "<br>";
    ...
    function read_data_from_file($usetags, $fields=NULL, $alternate_filename = NULL) {
        ...
        //    $this->type = "mp3";
        //}
        $info = GetAllFileInfo($this->filename, $this->type) or die("Error while processing $this->filename");
        if (!empty($info[error])) {
            echo "<b>Error: $info[error]</b><br>";
        }
        /*
        ...
        */
        ...
        if (!$usetags) {
            ...
            elseif($info[fileformat]=="ogg") {
                //echo "it's an OGG";
                $track = $info[comments][tracknum][0];
                $year = "NULL";
                $layer = "NULL";
            }
    ...
    function write_data_to_file() {
        require_once("id3/getid3.php");
        require_once("id3/getid3.putid3.php");
        require_once("database.inc.php");
        if (empty($this->type)) {
        ...
        if ($this->type == "mp3") {
            ...
            if(!WriteID3v1($this->filename, $this->songtitle, $this->artist_name, 
                        $this->album, $this->year, "", $this->genre_id, $this->track, TRUE)) {
                //die("error writing id3v1<br>");
                $error=TRUE;
            }
            // Title
            $id3['id3v2']['TIT2']['encodingid'] = 0;
            $id3['id3v2']['TIT2']['data'] = $this->songtitle;
            // Artist
            $id3['id3v2']['TPE1']['encodingid'] = 0;
            $id3['id3v2']['TPE1']['data'] = $this->artist_name;
            // Album
            $id3['id3v2']['TALB']['encodingid'] = 0;
            $id3['id3v2']['TALB']['data'] = $this->album_name;
            //Year
            $id3['id3v2']['TYER']['encodingid'] = 0;
            $id3['id3v2']['TYER']['data'] = $this->year;
            //Track
            $id3['id3v2']['TRCK']['encodingid'] = 0;
            $id3['id3v2']['TRCK']['data'] = $this->track;
            //Genre
            $id3['id3v2']['TCON']['encodingid'] = 0;
            $id3['id3v2']['TCON']['data'] = '(' . $this->genre_id . ')';

            if(!WriteID3v2($this->filename, $id3, 3, 0, FALSE, 0, TRUE)) {
                //die("error writing id3v2<br>");
                $error=TRUE;
            }
        }
        elseif ($this->type == "ogg") {
            require_once("id3/getid3.ogg.php");
            require_once("id3/getid3.ogginfo.php");
            $comments = Array (
    ...
    function print_info($skip_detail=NULL) {
        ...
        if (empty($skip_detail) && (!empty($this->large_album_cover) || $_SESSION[perms][p_select_edit])) {
            ...
            elseif ($_SESSION[perms][p_select_edit]) {
                echo    "<a href=\"albumpics.php?song_id=$this->song_id\">Get album art</a>";
            }
            echo    "</td></tr>";
        }
        echo    "<tr><th>Artist</th><td><a href=\"search.php?search_type=artist_name&searchFor=" .  urlencode($this->artist_name) . "\">$this->artist_name</a></td></tr>";
        echo    "<tr><th>Title</th><td><a href=\"songinfo.php?song_id=$this->song_id\">$this->songtitle</a></td></tr>";
        if ($_SESSION[perms][p_select_edit] && empty($skip_detail)) {
            echo "<tr><th>Filename</th><td>$this->filename</td></tr>";
        }
        echo "<tr><th>Album</th><td><a href=\"browse_album.php?selected_album=" .  urlencode($this->album_id) ."\">$this->album_name</a></td></tr>";
        echo    "<tr><th>Length</th><td>" . $this->length . "</td></tr>";
        if (empty($skip_detail)) {
            echo    "<tr><th>Stats</th><td>$this->type file, $this->bitrate_mode @ $this->bitrate" . "bps sampled at $this->frequency" . "hz</td></tr>" ;
        ...
        if ($_SESSION[perms][p_vote]) {
            echo    "<a title=\"$this->filename\" href=\"vote.php?action=vote&song_id=$this->song_id\"><img src=\"images/vote.png\" border=\"0\"></a>";
        }
        if ($_SESSION[perms][p_select_edit]) {
            echo    "<a href=\"admin_edit_record.php?checkbox_action=edit&song_id=$this->song_id\" title=\"Edit\"><img src=\"images/edit.png\" border=\"0\"></a> ";
        }
        echo    " <a href=\"history.php?song_id=$this->song_id\" title=\"Play History\">H </a>";
        if ($_SESSION[perms][p_download]) {
            echo    "<a href=\"download.php?song_id=$this->song_id\" title=\"Download\"><img src=\"images/save.png\" border=\"0\"></a> ";
        }
        if ($_SESSION[perms][p_select_edit]) {
            echo "&nbsp;&nbsp;&nbsp;<a href=\"admin_db_action.php?song_id=$this->song_id&action=delete\" title=\"Delete\"><img src=\"images/delete.png\" border=\"0\"></a>";
            echo    "| <a href=\"admin_db_action.php?song_id=$this->song_id&action=hide\" title=\"Hide\">H </a>";
            echo    "| <a href=\"admin_db_action.php?song_id=$this->song_id&action=readtag\" title=\"Read from ID3 tags on file\">R </a>";
            echo    "| <a href=\"admin_db_action.php?song_id=$this->song_id&action=writetag\" title=\"Write ID3 tag back to file\">W </a>";
        }
        echo "</td></tr>";
        echo "</table>";
    }
    ...
    function determine_root_path() {
        ...
        die("mp3 not found in mp3dir arrays!");
    }

    function guess_best_path($override_root_path=NULL) {
        global $distribute_style;
        if(empty($distribute_style)) {
            die("Sorry, you haven't set the distribute_style variable in config.inc.php");
        }
