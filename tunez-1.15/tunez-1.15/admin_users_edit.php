...
// I don't really care what all this commented code does . . . after the 30k
// of database changes we've made I think I'll just rewrite it :)
...
//die("needs to be rewritten for new authentication system");
// no longer!:)
// changing groups still needs to be fixed . . I should do that . .

if (empty($_POST))
{
        $query = "SELECT * FROM users LEFT JOIN access ON users.user_id=access.user_id LEFT JOIN groups ON access.group_id=groups.group_id WHERE users.user_id=" . $_GET[edit_user_id] . ";";
        $result=tunez_query($query);
        $query = "SELECT * FROM users LEFT JOIN permissions ON users.user_id=permissions.ug_id WHERE users.user_id=" . $_GET[edit_user_id] . " AND permissions.who='user'";
...
<tr>
<input type="hidden" name="edit_user_id" value="<?php echo $_GET[edit_user_id]; ?>">
...
else
{
	...
	if((!empty($_POST[newpw])) || !empty($_POST[newemail]) || !empty($_POST[newname]))
	{
                if (!empty($newpw))
        ...
        if (!empty($_POST[modmem])) {
                ...
                while ($row = mysql_fetch_array($grps)) {
                        ...
                        if (!empty($_POST[$html_grp]))
                                tunez_query("INSERT INTO access (user_id, group_id) VALUES (" . $_POST[edit_user_id] . ", " . $row[group_id] . ");");
                }
        }
